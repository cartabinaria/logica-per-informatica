...
theorem full_in_monotone: ∀A, A ∈ ℘ A := by
 -- Introduciamo ∀ e passiamo a dimostrare A ∈ ℘ A
 assume A: set
 -- Poichè ax_powerset2 ci dice che ∀Z,(Z⊆A → Z∈℘A), allora vale anche per A,
 -- quindi abbiamo che A⊆A → A∈℘A,
 -- quindi per dimostrare A ∈ ℘A possimamo ridurci a dimostrare A⊆A
 by ax_powerset2 it suffices to prove A⊆A
 by reflexivity_inclusion done
...
theorem powerset_monotone₁: ∀A B, A⊆B → ℘ A ⊆ ℘ B := by
 --Introduciamo i ∀ e passiamo a diostrare A⊆B → ℘ A ⊆ ℘ B
 assume A: set
 assume B: set
 -- Supponiamo A⊆B  e passiamo a dimostrare  ℘ A ⊆ ℘ B
 suppose A ⊆ B as H
 -- Espandiamo la definizione di sottoinsieme
 we need to prove ℘ A ⊆ ℘ B that is equivalent to ∀Z, Z ∈ ℘ A → Z ∈ ℘ B
 -- Fissiamo Z e passiamo a dimostare Z ∈ ℘ A → Z ∈ ℘ B
 assume Z: set
 -- Supponiamo  Z ∈ ℘ A   e passiamo a dimostrare   Z ∈ ℘ B
 suppose Z ∈ ℘ A
 -- Da Z ∈ ℘ A e ax_powerset1 (∀X, (X ∈ ℘A → X⊆A)) abbiamo che Z ⊆ A
 thus by ax_powerset1 we proved Z ⊆ A
 -- Da H (A⊆B) e transitivity_inclusion abbiamo che Z ⊆ B
 thus by H, transitivity_inclusion we proved Z ⊆ B
 -- Poichè (Z⊆B) allora (Z ∈ ℘B) per ax_powerset2
...
theorem powerset_monotone₂: ∀A B, ℘ A ⊆ ℘ B → A ⊆ B := by
 -- Introduciamo i ∀  e passiamo a dimostrare ℘ A ⊆ ℘ B → A⊆B
 assume A: set
 assume B: set
 -- Supponiamo ℘ A ⊆ ℘ B e passiamo a dimostrare A ⊆ B
 suppose ℘ A ⊆ ℘ B that is equivalent to ∀Z, Z ∈ ℘ A → Z ∈ ℘ B
 -- Da ∀Z, Z∈℘A → Z∈℘B e full_in_monotone (∀A, A∈℘A) abbiamo che A ∈ ℘B
 thus by full_in_monotone we proved A ∈ ℘ B
 -- Poichè A ∈ ℘B allora A⊆B, per ax_powerset1
...
theorem subset_intersection₁: ∀A B Z, Z ⊆ A ∩ B → Z ⊆ A ∧ Z ⊆ B := by
 -- Introduciamo i ∀ e passiamo a dimostrare  Z ⊆ A∩B → Z⊆A ∧ Z⊆B
 assume A: set
 assume B: set
 assume Z: set
 -- Supponiamo Z ⊆ A∩B e passiamo a dimostrare Z⊆A ∧ Z⊆B
 suppose Z ⊆ A ∩ B that is equivalent to ∀X, X∈Z → X∈A∩B as H
 -- Dobbiamo disostrare Z⊆A e Z⊆B quindi ci dividiamo in due sottoprove
 we split the proof
   -- Dobbiamo dimostrare Z⊆A
 . we need to prove Z ⊆ A that is equivalent to ∀Y, Y ∈ Z → Y ∈ A
   -- Fissiamo Y e passiamo a dimostrare ∀Y, Y ∈ Z → Y ∈ A
   assume Y: set
   -- Supponiamo Y∈Z e passiamo a dimostraare Y∈A
   suppose Y∈Z
   -- Da Y∈Z e H(∀X, X∈Z → X∈A∩B) abbiamo che Y∈A∩B
   thus by H we proved Y ∈ A∩B
   -- Da Y∈A∩B e ax_intersect1 abbiamo che Y∈A e Y∈B
   thus by ax_intersect1 we proved Y∈A as H₁ and Y∈B as H₂
   by H₁ done

   -- Dobbiamo dimostrare Z⊆B, analogo
...
theorem powerset_intersection₁: ∀A B, ℘ (A ∩ B) ⊆ ℘ A ∩ ℘ B := by
 -- Intoduciamo i ∀ e passiamo a dimostrare ℘ (A ∩ B) ⊆ ℘ A ∩ ℘ B
 assume A: set
 assume B: set
 -- Espandiamo la definizione di sottoinsieme
 we need to prove ℘ (A ∩ B) ⊆ ℘ A ∩ ℘ B that is equivalent to ∀Z, Z ∈ ℘ (A ∩ B) → Z ∈ ℘ A ∩ ℘ B
 -- Fisssimo Z e passiamo a dimostrare Z ∈ ℘ (A ∩ B) → Z ∈ ℘ A ∩ ℘ B
 assume Z: set
 -- Supponiamo Z ∈ ℘ (A ∩ B)  e passiamo a dimostrare  Z ∈ ℘ A ∩ ℘ B
 suppose Z ∈ ℘ (A ∩ B)
 -- Da  Z ∈ ℘(A∩B) e ax_powerset1 abbiamo che Z ⊆ A∩B
 thus by ax_powerset1 we proved Z ⊆ A ∩ B
 -- Da Z ⊆ A∩B e subset_intersection₁ abbiamo che Z⊆A  e Z⊆B
 thus by subset_intersection₁ we proved Z ⊆ A as H₁ and Z ⊆ B as H₂
 -- Da H₁(Z⊆A) e ax_powerset2 abbiamo che  Z ∈ ℘ A
 by H₁, ax_powerset2 we proved Z ∈ ℘ A as K₁
 -- Da H₂(Z⊆B) e ax_powerset2 abbiamo che  Z ∈ ℘ B
...
theorem subset_intersection₂: ∀A B Z, Z ⊆ A → Z ⊆ B → Z ⊆ A ∩ B := by
 -- Introduciamo i ∀ e passiamo a dimostrare Z⊆A → Z⊆B → Z⊆A∩B
 assume A: set
 assume B: set
 assume Z: set
 -- Supponiamo Z⊆A e passiamo a dimostrare Z⊆B → Z⊆A∩B
 suppose Z ⊆ A as H₁
 -- Supponiamo Z⊆B e passiamo a dimostrare Z⊆A∩B
 suppose Z ⊆ B as H₂
 -- Espandiamo la definizione di sottoinsieme
 we need to prove Z ⊆ A ∩ B that is equivalent to ∀X, X ∈ Z → X ∈ A ∩ B
 -- Fissiamo X e passiamo a dimostrare  X∈Z → X ∈ A∩B
 assume X: set
 -- Supponiamo X∈Z  e dimostriamo  X ∈ A∩B
 suppose X ∈ Z as K
 -- Da H₁(Z⊆A) e K abbiamo che X ∈ A
 by H₁, K we proved X ∈ A as K₁
 -- Da H₂(Z⊆B) e K abbiamo che X ∈ B
 by H₂, K we proved X ∈ B as K₂
 we need to prove X ∈ A ∩ B
 by K₁, K₂, ax_intersect2 done
...
theorem powerset_intersection₂: ∀A B, ℘ A ∩ ℘ B ⊆ ℘ (A ∩ B) := by
 ...
 -- Supponiamo Z ∈ ℘A∩℘B e passiamo a dimostrare  Z∈℘(A∩B)
 suppose Z ∈ ℘ A ∩ ℘ B
 -- Da Z ∈ ℘A∩℘B e ax_intersect1 abbiamo che (Z ∈ ℘A) e (Z ∈ ℘B)
 thus by ax_intersect1 we proved Z ∈ ℘ A as H₁ and Z ∈ ℘ B as H₂
 -- Da H₁(Z ∈ ℘A) e ax_powerset1 abbiamo che Z⊆A
 by H₁, ax_powerset1 we proved Z ⊆ A as K₁
 -- Da H₂(Z ∈ ℘B) e ax_powerset1 abbiamo che Z⊆B
